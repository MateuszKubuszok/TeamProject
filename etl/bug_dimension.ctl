...
out_table = BugDimension.table_name

bulk_load_file = "data/#{out_table}.csv"
columns   = [ :date_id, :bug_id, :project_id, :short_description, :description, :commentary ]
separator = "\t"
...
source :in, {
  ...
  target: in_db,
  table:  in_table,
  select: 'bugs.id AS `bug_id`, bugs.project_id, bugs.short_description, bugs.description, bugs.commentary'
}, columns
